(function () {
  ...
  function installRapidActionGuard() {
    ...
    document.addEventListener("click", function (event) {
      ...
      const text = String(button.textContent || button.value || button.getAttribute("aria-label") || "")
        .trim()
        .toLowerCase();
      const inlineAction = String(button.getAttribute("onclick") || "").toLowerCase();
      const isAction = actionWords.some(word => text.includes(word) || inlineAction.includes(word.replace(/\s+/g, "")));
